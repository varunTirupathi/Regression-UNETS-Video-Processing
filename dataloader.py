...
class Video3DDataset(Dataset):
	def __init__(self, video_path, train=False, img_transform=img_transform, mask_transform=mask_transform, mask_filter='CannyEdgeDetection', evaluate=False):
		...
		self.min_size = self.find_min_video_size()
		self.mask_transform = SampleEntropy(self.min_size-1)

	def find_min_video_size(self):
		videos = [x for x in os.listdir(self.video_path)]		# List all the videos at given path.
		...
		for vfp in self.videos_full_path:
			cap = cv2.VideoCapture(vfp)
			min_size = min(min_size, int(cap.get(cv2.CAP_PROP_FRAME_COUNT)))			# Find video with minimum number of frames.
			cap.release()
		return min_size

	def __len__(self):
		return len(self.videos_full_path)

	def __getitem__(self, index):
		frame = []
		cap = cv2.VideoCapture(self.videos_full_path[index])
		...
		return frame, self.mask_transform(frame)


if __name__ == '__main__':
	img_dir = './dataset/contour/train/'
	mask_dir = './dataset/contour/train_masks/'
	# dataset = ImageDataset(img_dir, mask_filter='CannyEdgeDetection')
	# img, mask = dataset[0]
	# img = img.permute(1, 2, 0)
	# mask = mask.permute(1, 2, 0)
	# result = torch.cat([img, mask], axis=1)
	...
	for i in range(len(dataset)):
		img, mask = dataset[i]
		import ipdb; ipdb.set_trace()
		# cv2.imshow('frame', mask.permute(1, 2, 0).numpy())
